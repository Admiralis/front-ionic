...
class ComputerRepositoryMock {
     ...
     * @param serialNumber
     */
    findBySerial(serialNumber: string): Promise<Computer> {
        let computer = computers.find(computer => computer.serialNumber === serialNumber);
        if (computer) {
            return Promise.resolve(computer);
        }
        return Promise.reject(new Error("Computer not found"));
    ...
    save(newComputer: NewComputer): Promise<Computer> {
        const index = computers.findIndex(c => c.serialNumber === newComputer.serialNumber);
        if (index > -1) {
            computers[index] = {
...
export default Object.freeze(new ComputerRepositoryMock());